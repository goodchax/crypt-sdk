...
public class AntCertificationUtil {
    ...
    public static boolean isTrusted(String cert, String rootCertContent) {
        ...
        try {
            ...
            SlansLogger.logBizError(("读取证书失败"));
        ...
        try {
            ...
            SlansLogger.logBizError(("读取根证书失败"));
            throw new RuntimeException(e);
        }
    ...
    private static boolean verifyCert(X509Certificate cert, X509Certificate[] rootCerts) {
        try {
            ...
            SlansLogger.logBizError(("证书已经过期"));
            return false;
        } catch (CertificateNotYetValidException e) {
            SlansLogger.logBizError(("证书未激活"));
            return false;
        }
        ...
        if (issuer == null) {
            SlansLogger.logBizError(("证书链验证失败"));
            return false;
        }
        try {
    ...
    private static boolean verifyCertChain(X509Certificate[] certs, X509Certificate[] rootCerts) {
        ...
        if (!sorted) {
            SlansLogger.logBizError(("证书链验证失败：不是完整的证书链"));
            return false;
        }
    ...
    public static String getRootCertSN(String rootCertContent) {
        ...
        try {
            ...
            SlansLogger.logBizError(("提取根证书失败"));
        }
        return rootCertSN;
     ...
     */
    public static X509Certificate getCertFromPath(String certPath) throws SlansApiException {
    ...
    public static X509Certificate getCertFromContent(String certContent) throws SlansApiException {
        try {
    ...
    public static String getCertSN(X509Certificate cf) throws SlansApiException {
        try {
            MessageDigest md = MessageDigest.getInstance("MD5");
